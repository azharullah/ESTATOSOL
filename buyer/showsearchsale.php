...
<?php
	...
	if(isset($_POST['submit2']))
	{
		require "../login/connectdb.php";
		 ...
		 $mcity = $_POST['city2'];
		 $mtype = $_POST['type2'];
		 $maxarea = $_POST['maxarea2'];
		 $maxcost = $_POST['maxprice2'];
	
		if($mtype == "All")
		{
			if(!empty($_POST['maxarea2']) && empty($_POST['maxprice2']))
			{
				//echo "maxarea set, maxcost not";
				$sql = "SELECT * FROM PROPERTY WHERE AREA<=$maxarea AND ROS='S' ORDER BY TIMEDATE DESC";
			...
			{	
				...
				$sql = "SELECT * FROM PROPERTY WHERE PRICE<=$maxcost AND ROS='S' ORDER BY TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later...</div>');
			...
			if(empty($_POST['maxarea2']) && empty($_POST['maxprice2']))
			{
				...
				$sql = "SELECT * FROM PROPERTY WHERE ROS='S' ORDER BY TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later...</div>');
			...
			if(!empty($_POST['maxarea2']) && !empty($_POST['maxprice2']))
			{
				...
				$sql = "SELECT * FROM PROPERTY WHERE PRICE<=$maxcost AND AREA<=$maxarea AND ROS='S' ORDER BY TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later...</div>');
		...
		if($mtype == "Flat")
		{
			...
			{
				//echo "maxarea set, maxcost not";
				$sql = "SELECT * FROM PROPERTY,FLAT WHERE PROPERTY.AREA<=$maxarea AND PROPERTY.ROS='S' AND FLAT.BHK=$bhk ORDER BY PROPERTY.TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later1...</div>'.mysql_error());
				}
				while($row = mysql_fetch_assoc($query))
				{
					$sno = $row['SNO'];
			...
			{	
				//echo "maxarea not, maxcost set";
				$sql = "SELECT * FROM PROPERTY,FLAT WHERE PROPERTY.PRICE<=$maxcost AND PROPERTY.ROS='S' AND FLAT.BHK=$bhk ORDER BY PROPERTY.TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later2...</div>');
				}
				while($row = mysql_fetch_assoc($query))
				{
					$sno = $row['SNO'];
			...
			{
				//echo "maxarea not, maxcost not";
				$sql = "SELECT * FROM PROPERTY,FLAT WHERE PROPERTY.ROS='S' AND FLAT.BHK=$bhk ORDER BY PROPERTY.TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later3...</div>'.mysql_error());
				}
				while($row = mysql_fetch_assoc($query))
				{
					$sno = $row['SNO'];
			...
			{
				//echo "maxarea set, maxcost set";
				$sql = "SELECT * FROM PROPERTY,FLAT WHERE PROPERTY.PRICE<=$maxcost AND PROPERTY.AREA<=$maxarea AND PROPERTY.ROS='S' AND FLAT.BHK=$bhk ORDER BY PROPERTY.TIMEDATE DESC";
				$query = mysql_query($sql,$mysql_conn);
				if(!$query)
				{
					die('<div class="errmsg">Cannot get the properties, try again later4...</div>');
				}
				while($row = mysql_fetch_assoc($query))
				{
					$sno = $row['SNO'];
